...
nm_setting_ip_config_get_num_dhcp_reject_servers (NMSettingIPConfig *setting)
{
	...
	return nm_g_array_len (priv->dhcp_reject_servers);
}
 ...
 *
 * Returns: (transfer none): the DHCP reject server at index @idx
 *
 * Since: 1.28
 **/
const char *
nm_setting_ip_config_get_dhcp_reject_server (NMSettingIPConfig *setting, guint idx)
{
	NMSettingIPConfigPrivate *priv;

	g_return_val_if_fail (NM_IS_SETTING_IP_CONFIG (setting), NULL);
	priv = NM_SETTING_IP_CONFIG_GET_PRIVATE (setting);
	g_return_val_if_fail (priv->dhcp_reject_servers && idx < priv->dhcp_reject_servers->len, NULL);

	return g_array_index (priv->dhcp_reject_servers, const char *, idx);
}
 ...
 * @setting: the #NMSettingIPConfig
 * @server: the DHCP reject server to remove
 *
 * Removes @server from the reject list.
 *
                                                         ...
                                                         const char *server)
{
	...
	for (i = 0; i < priv->dhcp_reject_servers->len; i++) {
		if (nm_streq (server, g_array_index (priv->dhcp_reject_servers, const char *, i))) {
			g_array_remove_index (priv->dhcp_reject_servers, i);
			_notify (setting, PROP_DHCP_REJECT_SERVERS);
			return TRUE;
		}
	}
